...
contextBridge.exposeInMainWorld('api', {
  ...
  updater: {
    ...
    onUpdateAvailable: (cb: (version: string) => void): (() => void) => {
      ...
      return () => { ipcRenderer.removeListener(IPC_UPDATER_UPDATE_AVAILABLE, handler) }
    },
    onUpdateDownloaded: (cb: (version: string) => void): (() => void) => {
      const handler = (_e: IpcRendererEvent, version: string): void => { cb(version) }
      ipcRenderer.on(IPC_UPDATER_UPDATE_DOWNLOADED, handler)
      return () => { ipcRenderer.removeListener(IPC_UPDATER_UPDATE_DOWNLOADED, handler) }
    },
    onUpToDate: (cb: () => void): (() => void) => {
      const handler = (): void => { cb() }
      ipcRenderer.on(IPC_UPDATER_UP_TO_DATE, handler)
      return () => { ipcRenderer.removeListener(IPC_UPDATER_UP_TO_DATE, handler) }
    },
    onError: (cb: (message: string) => void): (() => void) => {
      const handler = (_e: IpcRendererEvent, message: string): void => { cb(message) }
      ipcRenderer.on(IPC_UPDATER_ERROR, handler)
      return () => { ipcRenderer.removeListener(IPC_UPDATER_ERROR, handler) }
    }
  },
  ...
  onStatusChanged: (callback: (serverId: string, status: string) => void): (() => void) => {
    ...
    return () => { ipcRenderer.removeListener(IPC_WEBDAV_STATUS_CHANGED, handler) }
  }
})
